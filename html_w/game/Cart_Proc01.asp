...
<%
    ...
    End IF
    IF Flag = "add" THEN
	    IF LCase(Game_Type)  <> "power" and LCase(Game_Type)  <> "live"  and LCase(Game_Type)  <> "high" and LCase(Game_Type)  <> "dal" and LCase(Game_Type)  <> "aladin" and LCase(Game_Type)  <> "dari" and LCase(Game_Type)  <> "virtuals" and LCase(Game_Type)  <> "powers" Then
...
'''''''''''''''''''''''''''''''''''''''''''''''''''승무패와 핸디캡 동일경기 크로스 불가하게 추가됨''''''''''''''''''''''''''''''''''''''''
    ...
    End IF     						
		...
		If Not sRs.eof Then
			...
			Response.Write "parent.document.getElementById('"&tdID&"').className='CssSelect';"
			Response.Write "</script>"
	...
	If Not sRs.eof Then
		...
		IF BetNum = 1 THEN
			...
			Choice = "<font color=84ff00>승</font>"
			teamClass1 = "menubar_1_1_1"
			teamClass2 = "menubar_default"
		ELSEIF BetNum = 0 THEN
			Benefit = sRs("IG_DrawBenefit")
			Choice = "<font color=84ff00>무</font>"
			teamClass1 = "menubar_default"
			teamClass2 = "menubar_default"					
		ELSEIF BetNum = 2 THEN
			Benefit = sRs("IG_Team2Benefit")
			Choice = "<font color=84ff00>패</font>"
			teamClass1 = "menubar_default"
			teamClass2 = "menubar_1_1_1"					
		END IF		
				
			'returnVal = returnVal &  "<table width='200' height='52' border='0' cellpadding='0' cellspacing='0' background='' align='center'>"
			...
			returnVal = returnVal &  "<tr><td rowspan='3' style='font-size: 11px;'><input type='radio' id='ISAR' name='ISAR' style='Display:None;' value='"+ CStr(ICT_GameNum) +"'></td><td height='21' valign='bottom'>"
			returnVal = returnVal &  "<table width='200' height='13' border='0' align='left' cellpadding='0' cellspacing='0'>"
			returnVal = returnVal &  "<tr><td width='10'></td>"
			if CStr(BetNum)="1" then
			returnVal = returnVal &  "<td style='font-size: 11px;color: rgb(254, 239, 0);'>"+ team1 +" &nbsp;VS</td>"
			else
			returnVal = returnVal &  "<td style='font-size: 11px;color: rgb(255, 255, 255);'>"+ team1 +" &nbsp;VS</td>"
			end if
			returnVal = returnVal &  "<td width='18' ><input id='valuevalue' type='hidden' value='"&ICT_Idx&"'/><input id='valuevalue2' type='hidden' value='"&BetNum&"'/><a href=javascript:subCart('"&Session("SD_ID")&"','"&ICT_Idx&"','"&ICT_GameNum&"','"&BetNum&"','"&tdID&"');><img src='/images/cart/bet_del.png' width='11' height='11' border='0'/></a></td>"
			returnVal = returnVal &  "</tr></table></td></tr><tr><td height='6'></td></tr><tr><td valign='top'>"
			returnVal = returnVal &  "<table width='200' height='13' border='0' align='left' cellpadding='0' cellspacing='0'>"
			returnVal = returnVal &  "<tr><td width='10'></td>"
			if CStr(BetNum)="2" then
			returnVal = returnVal &  "<td width='130' style='font-size: 11px;color: rgb(254, 239, 0);'>"+ team2 +"</td>"
			else
			returnVal = returnVal &  "<td width='130' style='font-size: 11px;color: rgb(255, 255, 255);'>"+ team2 +"</td>"
			End if
			returnVal = returnVal &  "<td width='44' style='font-size: 11px;color: rgb(255, 228, 0);'>"+ choice +"&nbsp;"+ cstr(Benefit) +"</td>"
