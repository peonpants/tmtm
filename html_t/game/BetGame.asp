...
<%
	...
	%>
		<!-- #include file="BetGame_Type_live.asp" -->
	<%
		elseIF LCase(Game_Type)  = "dal" THEN 
	%>
		<!-- #include file="BetGame_Type_dal.asp" -->
	<%
		elseIF LCase(Game_Type)  = "power" THEN 
	%>
		<!-- #include file="BetGame_Type_power.asp" -->
	<%
		elseIF LCase(Game_Type)  = "casino" THEN 
	%>
		<!-- #include file="BetGame_Type_casino.asp" -->
	<%
		elseIF LCase(Game_Type)  = "casino_help" THEN 
	%>
...
<script type="text/javascript">
	...
	function addCart(obj, iuid,igidx,betnum,Gty,Gmemo,handi,status)
	{
		if (status==='S') {
	...
	function addCart_live(iuid,igidx,betnum,Gty,Gmemo,handi,btnID) 
	{
		if (Gty == 'live' || Gty == 'power' || Gty == 'dari' || Gty == 'powers' ) {	
				if($("#"+btnID).prop('class').indexOf('Selected') >= 0){
				}else{
				}
		}else{
			for(var i=0;i<3;i++) {
				setTimeout(subCartAll, 800);	

				if($("#id"+betnum+"_" + igidx).prop('class').indexOf('Selected') >= 0){
					$("#id"+betnum+"_" + igidx).addClass('Selected').removeClass('normal');
				}else{
					$("#id"+betnum+"_" + igidx).removeClass('Selected').addClass('normal');
				}
				if(document.getElementById("id"+betnum+"_" + igidx) != null){
					document.getElementById("id"+betnum+"_" + igidx).className = "normal";            
				}            
			}

			if(document.getElementById("id"+betnum+"_" + igidx) != null)
				document.getElementById("id"+betnum+"_" + igidx).className = "Selected";          
		}	
		ProcFrm.location.href = "/Game/Cart_Proc01.asp?Game_Type=<%=Game_Type%>&Flag=add&ICT_ID="+iuid+"&IG_Idx="+igidx+"&ICT_BetNum="+betnum+"&GMemo="+Gmemo+"&ig_handicap="+handi;
	}
